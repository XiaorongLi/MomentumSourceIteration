...
The corresponding code is in the file "`iteration.py`". A separate programm called CTF is used to solve the Navier-Stokes equations for the simplified system. The output of the CTF program is pure text files. Therefore, it is necessary to create a few functions to parse these text file and extract information of interest. I don't talk about these functions in details. They can also be found in the file "`iteration.py`".

It is worth pointing out that [proper data processing](https://github.com/XiaorongLi/CFD_data_processing) is necessary preceeding the current work.
